...
const Tender: React.FC = () => {
  ...
  const mockServerValidation = async (content: string) => {
    ...
    return new Promise<"Approved" | "Rejected">((resolve) => {
      setTimeout(() => {
        const isValid = requiredPhrases.every((phrase) => {
          return content.toLowerCase().includes(phrase.toLowerCase());
        });
        resolve(isValid ? "Approved" : "Rejected");
      }, 1000);
    });
  };
  ...
  const checkForNameInExcel = async (data: any[]) => {
    const flatData = data.flatMap((row) =>
      Object.keys(row).map((val) => String(val))
    );
  ...
  return (
    <>
      <div className="min-h-screen bg-gray-50 p-6 flex flex-col items-center">
        ...
        {/* Validation Result */}
        {loading && (
          <p className="mt-4 text-blue-500 animate-pulse">Validating file...</p>
        )}
        {validationResult && (
          <motion.div
